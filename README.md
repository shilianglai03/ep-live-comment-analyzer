...
- 批量评论导入：支持一行一条评论、CSV / TSV，以及常见采集脚本导出的 JSON Lines。
- 直播事件适配：提供通用事件规范化层，便于后续接入 TikTokLive、B 站弹幕、本地 WebSocket 或其他采集脚本。
- 评论意图识别：覆盖购买意向、价格优惠、规格尺码、库存颜色、物流发货、质量售后、风险质疑和普通互动。
- 决策优先级判断：输出决策类型、置信度和命中语义信号，优先识别价格、售后、履约、尺码和购买阻断问题。
- 场控动作建议：为高价值评论生成“先报到手价、先给售后兜底、先讲履约确定性”等处理动作、目标和建议时效。
- 商品相关性过滤：区分当前讲解商品、其他商品问题和直播间噪声，避免跑题评论干扰指标。
- 商品知识配置：商品信息、别名和相关信号集中在目录模块中，运行设置页支持编辑当前商品资料并本地持久化。
- 智能回复参考：先生成本地模板回复，AI 可用时自动增强为更自然的话术。
- AI 多轮修改：可对单条回复输入修改要求，例如更短、更亲切、弱化促单或强调发货。
- 回复归档：将已处理回复归档，可标记已口播、疑似转化、继续追问或未解决，并支持搜索、筛选和导出 CSV。
- 实时看板：展示评论量、有效评论、待回复、购买信号、风险质疑、高频热词、决策压力和场控提醒。
...
该检查会走完整的“商品相关性过滤、真实意图分类、优先级打分”链路，覆盖价格咨询、口语化同义表达、售后换货、尺码咨询、包邮履约、弱价格感叹、购买入口、风险质疑和离题评论隔离等样例。

检查批量导入解析是否兼容常见字段和 JSON Lines：

```powershell
npm run check:import
```

检查直播评论事件适配层是否兼容常见 JSON 事件结构：
